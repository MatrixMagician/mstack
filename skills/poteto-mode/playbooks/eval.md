...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript at `~/.claude/projects/<escaped-cwd>/<sessionId>.jsonl`, per `../references/transcripts.md`. Do not glob across `~/.claude/projects/*/`; that crosses workspace boundaries and reads private chats from unrelated projects. Look at the `tool_use` blocks to see which files each candidate actually opened, and at `attributionSkill` to see which skills really drove the turn rather than which ones the candidate says it used. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
