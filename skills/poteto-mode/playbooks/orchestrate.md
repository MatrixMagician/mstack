...
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn has the full `Agent` schema including `isolation`). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always `isolation: "worktree"`. Every worker runs on this machine, so runtime verification (`control-ui`, `control-cli`), local transcripts, simulators, and local auth are all directly reachable — there is no partition to plan around and no need to inline what a brief could point at. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different tier from its worker, and never on the same agent — with one model family, tier spread and an independent context are the decorrelation you have.
...
- `inbox/` holds completion pointers. `gates.md` parks decision gates (question, options, default on no answer) so a completion flood cannot wipe AskUserQuestion state.
...
- Critical sections you finish first: authoring a brief, a stack operation, a conflict decision, writing a decision gate, updating ledger or frontier.
...
- A drain turn ends with the three lines from `orch status`: counts against the states, what changed, decision gates open. Detail lives in `status.md`; the full reply contract applies at checkpoints and close.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks at this scale are heavy: run them from a dedicated worktree so the rest of the fleet keeps working while one checkout churns.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and the session's status in the agent view. Transcript mtime is not liveness.
...
- After a Claude Code restart: subagents are dead, and background sessions are dead too if the machine went down. They survive sleep, not shutdown. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach surviving work by PR and branch rather than agent id, `claude respawn` / `--resume` what can be recovered, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
...
**Reply:** at checkpoints and close: the predicate and the count against it from `units.tsv` and `ledger.tsv`, tracks and what each landed, the frontier (PR list plus SHAs), verdicts summary, what was abandoned and why, decision gates awaiting the human (the only asks), the store path, and the trail path. Numbers from the tables, not narrative. Include PR links.
