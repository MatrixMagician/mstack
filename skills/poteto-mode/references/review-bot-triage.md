...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles comments from a PR review bot — `claude-code-action`, CodeRabbit, an agentic security review, or whatever else your repo runs. The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.

The patterns below are written against `claude-code-action` as the worked example because it is the bot an mstack repo most often has installed. Nothing in the rubric depends on which bot posted: classify on what the comment claims and what the code shows, never on the author.

## Decision rubric

Classify each the review bot thread before acting:
...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the the review bot comment is only restating that a shared visual default changed.
...
- Confidence: candidate
- Skip when: the review bot flags an export, component, helper, or file as unused, and the active forge's PR list and diffs, upper-stack diffs, or PR context show it is used by a later PR in the stack.
...
- Confidence: candidate
- Skip when: Practically never. When a diff replaces native browser behavior with a manual equivalent (native sticky → JS-positioned clones, native scroll targeting → forwarded wheel/touch events, paint-order occlusion → masks/clip-path), the bot's logic-bug findings against that code have been consistently legitimate.
...
- Skip when: Never skip the verification itself; it costs one command. When a PR
  ...
  a SKILL.md, snapshot of doc wording), and the review bot claims "the test no longer
