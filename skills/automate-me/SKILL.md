...
This skill orchestrates three others: an inline mining pass (see step 1), Claude Code's `skill-creator` skill (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out: `~/.claude/projects/<escaped-cwd>/*.jsonl`, per `../poteto-mode/references/transcripts.md`. Use only that directory. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Use Claude Code's `skill-creator` skill to author the skill (`plugin-dev` instead when the target is a plugin component). Placement:
...
- Claude Code's `skill-creator` skill: skill authoring process and writing guidelines.
