...
Transcripts live at `~/.claude/projects/<escaped-cwd>/<sessionId>.jsonl`, one file per session. A transcript is not one chat message per line: most lines are session metadata, and a `user` line is often a tool result rather than something the human said. Read `../poteto-mode/references/transcripts.md` before parsing one — the path-escaping rule and the line-type filter are both easy to get wrong in ways that fail silently.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time (`ls -t`) and never by session-id name, to follow `../poteto-mode/references/transcripts.md` for location and parsing, to grep the topic first and then read only the matching sessions and only their relevant regions, and to skip the current session plus obvious noise (`isSidechain` lines, eval and test sessions). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the session id. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
