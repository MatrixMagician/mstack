...
Launch all reviewers in a single message using the Task tool. Use the `interrogate reviewers` list from `.claude/rules/mstack-models.md` when that file exists, one reviewer per entry, extending or shrinking the Reviewer A/B/C labels below to the configured entry count; otherwise use the table defaults.
